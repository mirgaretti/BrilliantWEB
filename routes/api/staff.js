...
async function getStaffHandler(req, res) {
    try {
        ...
        if (scope == 'full') {
            ...
            selectSQL = 'SELECT id, Active, Name, Role, Code, PublishTopicIn, PublishPayloadIn, PublishTopicOut, PublishPayloadOut, Floor, WebRelay, ScheduleRelay, CardType FROM Staff';
            [rows] = await connection.execute(selectSQL);
        }
